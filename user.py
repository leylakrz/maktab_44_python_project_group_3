...
class User:
    ...
    def write_message(self, receiver=None, body=None, title=None, date=datetime.now().date()):
        self.check_login_status()  # check login status to allow to write a message
        print()
        ...
        for item in write_data['data']:
            if item["User"] == self.username:
                item["Draft"].append(json.dumps(message.__dict__))

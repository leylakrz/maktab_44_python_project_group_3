...
def main():
    ...
    def open_menu_tabs(messenger_menu, user):
        ...
        def delete(box, entry_box):
            try:
                ...
                if delete_text == "Successfully Deleted":  # check whether deleting is successful or not
                    ...
                    logger.warning("attempt to delete nonexistent message")
        ...
        def read():
            try:
                ...
                if message_content == "Message not Found":  # check whether the message exists or not
                    ...
                    logger.warning("attempt to read nonexistent message")
                else:
                    # create read window
                    read_window = Tk()
        ...
        def write_message(receiver_entry, title_entry, body_entry):
            ...
            if receiver == "":  # check whether receiver is empty or not
                ...
                logger.warning("attempt to send message with no receiver")
            else:
                ...
                if written_message == "Receiver not Found":  # check whether receiver exists or not
                    logger.warning("attempt to send message with nonexistent receiver")
        ...
        def send_message(receiver_entry, title_entry, body_entry, num_message=None, window=None):
            ...
            if message_status == 'Unread':  # check status of message
                logger.warning("attempt to send message with not existing receiver")
        ...
        sign_out_button = Button(sign_out_tab, text="Sign Out", bg='pink', font=("times new roman", 20, "bold"),
                                 command=lambda: sign_out())
        sign_out_button.grid(row=2, column=2)
        # show account menu
        root.mainloop()
    ...
    def register():
        ...
        if username == "" or username.isspace() or password == "" or password.isspace():
            logger.warning("attempt to sign in with empty username or password")
    ...
    bg_photo = ImageTk.PhotoImage(file="main2.png")
    bg_lbl = Label(app, image=bg_photo).pack()
    # Design register tab
    user_icon = ImageTk.PhotoImage(file="User_icon2.png")
    # put username label and entry

    ttk.Label(register_tab, image=user_icon).grid(row=0)
    ttk.Label(register_tab, text='password').grid(row=2, column=1, padx=20, pady=5)
    ttk.Label(register_tab, text='password', compound=LEFT)

    username_register_entry = ttk.Entry(register_tab, font=("times new roman", 15))
    username_register_entry.grid(row=0, column=1)

    # put password label and entry
    pass_icon = ImageTk.PhotoImage(file="pass1.png")
    ttk.Label(register_tab, text='password', image=pass_icon).grid(row=2, column=0)
    password_register_entry = ttk.Entry(register_tab, show="*", font=("times new roman", 15))
    password_register_entry.grid(row=2, column=1, padx=20)

 # Button design

    login_btn = ImageTk.PhotoImage(file="btn2.png")
    img_lbl = Label(image=login_btn)
    img_lbl.pack(pady=20)

    # put register button
    register_button = ttk.Button(register_tab, image=login_btn, text='Register', width=15, command=register)
    register_button.grid(row=5, column=1, padx=20)
    ...
    # Design login tab
    # put username label and entry
    ttk.Label(login_tab, text='username', image=user_icon).grid(row=0)
    username_login_entry = ttk.Entry(login_tab, font=("times new roman", 15))
    username_login_entry.grid(row=0, column=1)

    # put password label and entry
    ttk.Label(login_tab, text='password', image=pass_icon).grid(row=2)
    password_login_entry = ttk.Entry(login_tab, show="*", font=("times new roman", 15, "bold"), )
    password_login_entry.grid(row=2, column=1,padx=20)

    login_btn1 = ImageTk.PhotoImage(file="btn3.png")
    img_lbl = Label(image=login_btn)
    img_lbl.pack(pady=20)

    # put login button
    login_button = Button(login_tab, image=login_btn1, command=lambda: login_user(app))
    login_button.grid(row=5, column=1, padx=20)
    ttk.Label(login_tab, text="").grid(row=7, column=1)

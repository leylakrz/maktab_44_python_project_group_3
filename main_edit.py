...
def main():
    ...
    def open_menu_tabs(messenger_menu, user):
        ...
        def read():
            try:
                ...
                if message_content == "Message not Found":  # check whether the message exists or not
                    ...
                    # create read window
                    read_window = Tk()
                    read_window.geometry("700x750+350+0")
        ...
        def edit():
            try:
                ...
                update_window = Tk()
                update_window.geometry("700x750+350+0")
    ...
    # create register and login tabs
    tab_control_login = ttk.Notebook(app)
    tab_control_login.pack(pady=15)

    register_tab = Frame(tab_control_login, width=500, height=500)
    login_tab = Frame(tab_control_login, width=500, height=500)
    ...
    # Background Image
    Tk()
    bg_photo = ImageTk.PhotoImage(file="main.png")
    ...
    user_lbl = Label(login_frame,text="Username", image = user_icon, compound = LEFT, font =("times new roman" , 15, "bold"), bg = "white").grid(row =1 , column = 0, padx = 20, pady = 10)
    username_login_entry = Entry(login_frame, bd =5, relief = GROOVE, font=( "times new roman", 15)).grid(row = 1 , column = 1, padx = 20)


    # Logo Icon
    logo_icon = ImageTk.PhotoImage(file = "login2.png")

    logo_lbl = Label(login_frame, image = logo_icon, bd =0).grid(row = 0, columnspan = 2, pady =20 )

    # Password Icon
    pass_icon = ImageTk.PhotoImage(file="pass1.png")
    pass_lbl = Label(login_frame, text="Password", image=pass_icon, compound=LEFT, font=("times new roman", 15, "bold"),
                     bg="white").grid(row=2, column=0, padx=20, pady=10)
    password_login_entry = Entry(login_frame, bd=5, relief=GROOVE,show= "*", font=("", 15)).grid(row=2, column=1, padx=20)



    # Button Photo
    # btn_register = Button(login_frame,text = "Register", width = 15, font = ("times new roman" , 14, "bold"), bg = "yellow", fg= "red").grid(row = 3, column = 1, pady = 10)

    register_button = Button(register_tab,command=register, text="Register", width=15, font=("times new roman", 14, "bold"), bg="yellow",
                          fg="red").grid(row=3, column=1, pady=10)
    ...
    login_button = Button(login_tab, text='Login', command=lambda: login_user(app), width=15, font=("times new roman", 14, "bold"), bg="yellow",
                          fg="red")
    login_button.grid(row=3, column=1, pady = 10)
    ttk.Label(login_frame, text="").grid(row=6, column=1)
    # register_button = ttk.Button(register_tab, text='Register', command=register)
    ...
    username_register_entry = Entry(register_tab)
